...
import { currency } from "./function.js";
...
// 渲染購物車
function renderCarts(data = cartsData) {
  // 購物車有東西就渲染出來
...
// 購物車模板
function cartsTemplate(productsStr, totalCost) {
...
// 編輯購物車產品數量
function editItemNum(id, doSomething) {
  let num = 0;
  cartsData.carts.forEach((item) => {
    if (item.id === id) {
      num = item.quantity;
